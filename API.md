...
| `recaptchaToken` | string | No | reCAPTCHA v3 token |
...
| `zipCode` | string | No | 5-digit ZIP code |
| `source` | string | No | Where they subscribed from |
| `recaptchaToken` | string | No | reCAPTCHA v3 token |
...
| 409 | Already subscribed | Email already in database |
...
| 429 | Rate limited | Too many checkout attempts |
...
| 429 | Rate limited | Too many lookups |
...
Rate limits are enforced per IP address. When exceeded:
